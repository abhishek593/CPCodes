/* Credits - Codeforces user - saketh
...
bitset<MAX_SUM + 1> subset_sum(InputIterator first, InputIterator last) {
    int sum = accumulate(first, last, 0);
    assert(sum <= MAX_SUM);

    static vector<int> count(MAX_SUM + 1);

    for (InputIterator iter = first; iter != last; iter++)
        count[*iter]++;

    bitset<MAX_SUM + 1> result;
    result[0] = 1;
